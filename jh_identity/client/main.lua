...
RegisterNetEvent('Jh_identity:showRegisterIdentity')
AddEventHandler('Jh_identity:showRegisterIdentity', function()
	Citizen.Wait(2000)
	EnableGui(true)
end)

RegisterNetEvent('Jh_identity:identityCheck')
AddEventHandler('Jh_identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent('Jh_identity:saveID')
AddEventHandler('Jh_identity:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	if reason == "" then
		TriggerServerEvent('Jh_identity:setIdentity', data, myIdentifiers)
		EnableGui(false)
		Citizen.Wait(500)
		TriggerEvent('esx_skin:openSaveableMenu', myIdentifiers.id)
	elseif reason == "Inakzeptable Spielergröße!" then
		ESX.ShowNotification(reason)
		Citizen.Wait(1500)
		EnableGui(true)
	elseif reason == "Ungültiges Geburtsdatum!" then
		ESX.ShowNotification(reason)
		Citizen.Wait(1500)
		EnableGui(true)
	else
